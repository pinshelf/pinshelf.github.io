...
sw.addEventListener('fetch', (event) => {
    ...
    async function respond() {
        ...
        if (ASSETS.includes(url.pathname)
            || url.pathname === '/' // Very important for offline functionality
            || url.pathname === ''
        ) {
        ...
        if ((
            ...
            // Always try to execute the request first
            try {
                // Execute fetch request
                const response = await fetch(event.request)
                ...
                // Log cache write
                console.log(
                    '%cSW' + '%cFETCH' + '%c▶' + '%cAPI CACHE' + '%cWRITE '
                        + event.request.url,
                    SW_CSS, FETCH_CSS, DIVIDER_CSS, CACHE_CSS,
                    CACHE_WRITE_CSS,
                )
